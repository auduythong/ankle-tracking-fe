...
export const columnsRegion = (
  ...
  handleAdd: () => void,
  handleClose: () => void,
  setRecord?: (record: DataRegion) => void,
  setRecordDelete?: (record: DataRegion) => void,
  onViewClick?: (record: DataRegion) => void
) => {
  return [
    {
      sticky: 'left',
        ...
        <>
          {row.original.lat_location && row.original.long_location ? (
            <div className="flex items-center gap-2">
              ...
              <div>
                <div>{row.original.lat_location}</div>
                <div>{row.original.long_location}</div>
              </div>
            </div>
          ) : (
            'Chưa cập nhật'
